...
describe('<Books />', () => {
  ...
  it('displays author for each book', () => {
    // setup
      const props = [
      {title: 'Game of Thrones', author: 'George RR Martin', image: 'imageurl1'}, 
    ...
    // assertion
      expect(wrapper.text()).toEqual(expect.stringContaining('Game of Thrones, by George RR Martin'));
      expect(wrapper.text()).toEqual(expect.stringContaining('Mistborn, by Brandon Sanderson'));
      expect(wrapper.text()).toEqual(expect.stringContaining('The Two Towers, by JRR Tolkien'))
    })    
  })
